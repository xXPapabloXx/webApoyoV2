...
function RotatingModel({ modelPath, rotation }) {
...
export default function Rotation({ modelPath, rotation = [0, 0, 0] }) {
  return (
    <div className="m-0 w-full h-full">
      <Canvas camera={{ position: [0, 0, 0], fov: 45 }}>
        <Suspense fallback={null}>
          ...
          <gridHelper />
          <Bounds fit clip observe margin={0}>
            <Center>
              <RotatingModel modelPath={modelPath} rotation={rotation} />
            </Center>
          </Bounds>
        </Suspense>
        <OrbitControls />
